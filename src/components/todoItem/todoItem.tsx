    ...
    <div className={`${classes.todo}  ${props.todo.completed ? classes.todoFaded : null}`} key={props.todo.id}>
      ...
      <div>
        <div className={`${classes.todoChecked}`}>
          <input className={`form-check-input ${classes.todoCheckboxInput}`} type="checkbox" id={`complted-${props.todo.id}`} onChange={() => {props.checkHandler()}} defaultChecked={!!props.todo.completed} />
          <label className={`form-check-label ${classes.todoCheckboxLabel}`} htmlFor={`completed-${props.todo.id}`}>
            completed
          </label>
        </div>
      </div>
